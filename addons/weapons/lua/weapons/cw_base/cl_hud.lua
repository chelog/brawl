...
firemodeAl, firemodeFadeAfter = 255, 0
function SWEP:DrawHUD()
	...
	if not disableCustomHUD then
		...
		if not customHUD and GetConVarNumber("cw_customhud_ammo") <= 0 then
			if CurTime() > firemodeFadeAfter then
				firemodeAl = math.Approach( firemodeAl, 0, FrameTime() * 500 )
			end
			...
			if self.BulletDisplay and self.BulletDisplay > 0 then
				surface.SetDrawColor(0, 0, 0, firemodeAl)
				surface.SetTexture(Bullet)

				for i = 1, self.BulletDisplay do
					surface.DrawTexturedRect(x * 0.5 - 20 * i + self.BulletDisplay * 10 + 1, y - 29, 16, 16)
				end

				surface.SetDrawColor(220, 220, 220, firemodeAl)

				for i = 1, self.BulletDisplay do
					surface.DrawTexturedRect(x * 0.5 - 20 * i + self.BulletDisplay * 10, y - 30, 16, 16)
				end
			end
